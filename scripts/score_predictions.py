    ...
    for predictions_path in predictions_files:
        ...
        def score_multiple_predictions(row: dict):
            ...
            for pred_name, pred_smiles in row.items():
                if pred_name in ['protac_smiles', 'label_smiles']:
                    continue
                ...
                curr_scores[f"{pred_name}_fixed"] = True
                
                if pred_smiles == label_smiles:
                    curr_scores.update({f"{pred_name}_{metric}_fixed": v for metric, v in curr_scores.items()})
                ...
                if fixed_smiles is None:
                    curr_scores[f"{pred_name}_fixed"] = False
